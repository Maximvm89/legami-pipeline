...
resolved_local_root() ignoring the launcher's FLUMEN_PROJECT_ROOT env and from the
...
from flumen import config as cfgmod
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("FLUMEN_PROJECT_ROOT", raising=False)


def test_default_when_nothing_set(tmp_path, monkeypatch):
    _isolate(tmp_path, monkeypatch)
    c = _cfg()
    assert c.resolved_local_root().endswith("Flumen/LEGAMI".replace("/", __import__("os").sep))
...
def test_per_user_file_survives_blank_config(tmp_path, monkeypatch):
    ...
    _isolate(tmp_path, monkeypatch)
    cfgmod.save_local_root("E:/Flumen_4")
    assert cfgmod.load_local_root() == "E:/Flumen_4"
    assert _cfg().resolved_local_root() == "E:/Flumen_4"


def test_env_overrides_everything(tmp_path, monkeypatch):
    # The launcher exports FLUMEN_PROJECT_ROOT; subprocesses must honor it so the
    # turntable reads project_settings.json (and its template) from the right folder.
    _isolate(tmp_path, monkeypatch)
    cfgmod.save_local_root("E:/Flumen_4")
    monkeypatch.setenv("FLUMEN_PROJECT_ROOT", "E:/from_launcher")
    c = _cfg(local_root="/explicit/from/config")
